...
You interact with Ralph through a terminal (a window where you type commands). Ralph handles the rest — git, deployments, testing — so you don't need to know those systems.
...
**To check if you have it:** Open the Start menu, search "Git Bash," and see if it appears. If it does, you're set.

**To install:** Download from [git-scm.com](https://git-scm.com/download/win) and run the installer. Accept all defaults.
...
**To check if you have it:** Open Git Bash and run `node --version`. If you see a version number, you're set.
...
#### Setting up Windows Terminal for multi-tab sprints
...
After installing Windows Terminal, tell Ralph which profile to use by adding this to your `ralph.config.sh`:

```bash
export RALPH_WT_PROFILE="Git Bash"
```

**To find your profile name:** Open Windows Terminal, click the dropdown arrow next to the `+` tab button, and look at the profile names listed. Use the exact name (including capitalization) in the config above. Common names are `Git Bash`, `PowerShell`, or `Command Prompt`.
...
Windows Terminal isn't installed or isn't on your PATH. Install it from the Microsoft Store (search "Windows Terminal"). If it's installed but tabs still don't open, check that `wt` works by running `wt` in Git Bash. If it doesn't, try restarting Git Bash after installing.

**Profile name mismatch (wrong tab opens):**
The `RALPH_WT_PROFILE` value in `ralph.config.sh` doesn't match your Windows Terminal profile name exactly. Open Windows Terminal settings, find the exact profile name, and update the config. If you can't get it working, remove the `RALPH_WT_PROFILE` line entirely — Ralph will fall back to running tasks inline.

**Git credential errors:**
Git for Windows ships with **Git Credential Manager**, which saves your GitLab credentials automatically. If you're getting authentication errors, run:
```bash
git config --global credential.helper manager
```
Then run `git push` manually once from your project folder — it will open a browser window to authenticate with GitLab, and your credentials will be saved going forward.
