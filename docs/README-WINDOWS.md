...
| `ralph.config.sh` | Your project's configuration — dev URL, git settings, test commands |
...
| `stdlib/` | Coding patterns Ralph follows — ships with baseline patterns (security, testing, validation, docs, API routes). Customize for your stack and add project-specific patterns. |
...
### `stdlib/` — Coding patterns

Setup installs baseline patterns from the starter kit:

| File | What It Covers |
|---|---|
| `security.md` | Never hardcode secrets, .env patterns, pre-commit hooks |
| `documentation.md` | When and how to add docs as you build |
| `testing-playwright.md` | Selector priority, async waits, visual validation, debugging |
| `validation.md` | Schema as source of truth, safe field additions |
| `api-routes.md` | Auth → Validate → Execute → Respond, error handling |

These are starting points — edit them for your stack (e.g. replace Zod examples with Yup) and add project-specific patterns as new files (e.g. `graphql_patterns.md`, `databricks_patterns.md`). Ralph reads these during every task.
...
**Need help:**
Open an issue at [gitlab.zgtools.net/tpm_cdp_team/ralph-starter-kit](https://gitlab.zgtools.net/tpm_cdp_team/ralph-starter-kit/-/issues).
