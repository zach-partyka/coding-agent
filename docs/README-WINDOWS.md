...
```bash
bash ~/Documents/ralph-starter-kit/scripts/setup-project.sh
```
...
The `RALPH_WT_PROFILE` value in `ralph-config.md` doesn't match your Windows Terminal profile name exactly. Open Windows Terminal settings, find the exact profile name, and update the config. If you can't get it working, remove the `RALPH_WT_PROFILE` line entirely — Ralph will fall back to running tasks inline.

**Git credential errors:**
Git for Windows ships with **Git Credential Manager**, which saves your credentials automatically. If you're getting authentication errors, run:
```bash
git config --global credential.helper manager
```
Then run `git push` manually once from your project folder — it will open a browser window to authenticate, and your credentials will be saved going forward.
